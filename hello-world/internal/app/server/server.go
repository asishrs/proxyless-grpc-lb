...
func (s *server) SayHello(ctx context.Context, in *hello.HelloRequest) (*hello.HelloResponse, error) {
	logger2.Logger.Debug("Incoming Request", zap.String("name", in.Name))

	var host, err = os.Hostname()
	if err != nil {
		logger2.Logger.Debug("Unable to get hostname", zap.Error(err))
	}
...
func StartServer(port int) {
	...
	logger2.Logger.Info("Starting gRPC Server", zap.Int("port", port))

	lis, err := net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", port))
	if err != nil {
		logger2.Logger.Info("failed to listen", zap.Error(err))
	}
	...
	if err := grpcServer.Serve(lis); err != nil {
		logger2.Logger.Fatal("Failed to start gRPC Server")
	}
